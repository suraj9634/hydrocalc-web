...
class _AnalyticsScreenState extends State<AnalyticsScreen>
  ...
  // --- Particular Month Daily Trend State ---
  int _selectedMonthNumber = DateTime.now().month;
  int _selectedMonthYear = DateTime.now().year;
  final Map<int, double> _dailyAggregatedData = {};
  bool _isDailyMonthLoading = false;

  // --- Multi-Month Comparison State ---
  List<String> _selectedMultiMonths = [
  ...
  double _extractParameterValue(Map<String, dynamic> row, String paramKey) {
    if (paramKey == 'totalRg') {
      double r1 = (row['rg1'] as num?)?.toDouble() ??
          (double.tryParse("${row['rg1']}") ?? 0.0);
      double r2 = (row['rg2'] as num?)?.toDouble() ??
          (double.tryParse("${row['rg2']}") ?? 0.0);
      double r3 = (row['rg3'] as num?)?.toDouble() ??
          (double.tryParse("${row['rg3']}") ?? 0.0);
      return r1 + r2 + r3;
    }
    double val = (row[paramKey] is num)
        ? (row[paramKey] as num).toDouble()
        : (double.tryParse("${row[paramKey]}") ?? 0.0);
    return val < 0 ? 0.0 : val;
  }
  ...
  Future<void> _fetchAllSelectedDatesData() async {
    ...
    try {
      for (var date in _selectedDates) {
        ...
        _multiDateData[formattedDate] = records;
      }
    } catch (e) {
      debugPrint("Firestore multi-date error: $e");
    }
  ...
  Future<void> _fetchParticularMonthData() async {
    ...
    try {
      int daysInMonth =
          DateTime(_selectedMonthYear, _selectedMonthNumber + 1, 0).day;
      String monthStr = _selectedMonthNumber.toString().padLeft(2, '0');

      for (int day = 1; day <= daysInMonth; day++) {
        String targetDate =
            "${day.toString().padLeft(2, '0')}-$monthStr-$_selectedMonthYear";
        final snapshot = await FirebaseFirestore.instance
            .collection('barrage_readings')
        ...
        if (snapshot.docs.isNotEmpty) {
          ...
          for (var doc in snapshot.docs) {
            sum += _extractParameterValue(doc.data(), _selectedParameter);
            count++;
          }
          if (count > 0) _dailyAggregatedData[day] = sum / count;
        }
      ...
      debugPrint("Firestore month error: $e");
    }
  ...
  Future<void> _fetchMultiMonthData() async {
    ...
    try {
      for (String monthYearStr in _selectedMultiMonths) {
        ...
        for (int day = 1; day <= daysInMonth; day++) {
          String targetDate =
              "${day.toString().padLeft(2, '0')}-${mNum.toString().padLeft(2, '0')}-$year";
          final snapshot = await FirebaseFirestore.instance
              .collection('barrage_readings')
              .where('date', isEqualTo: targetDate)
              .get();
      ...
      debugPrint("Firestore multi-month error: $e");
    }
  ...
  Future<void> _fetchYearlyData() async {
    ...
    try {
      for (int i = 0; i < months.length; i++) {
        ...
        final snapshot = await FirebaseFirestore.instance
            .collection('barrage_readings')
            .where('date', isEqualTo: sampleDate)
            .get();
        ...
        if (snapshot.docs.isNotEmpty) {
          ...
          _monthlyAggregatedData[months[i]] = sum / snapshot.docs.length;
        }
      }
    } catch (e) {
      debugPrint("Firestore yearly error: $e");
    }
      ...
      for (int year in _selectedYears) {
        ...
        for (int i = 0; i < months.length; i++) {
          ...
          if (snapshot.docs.isNotEmpty) {
            ...
            yearMonths[months[i]] = sum / snapshot.docs.length;
          }
        }
        _multiYearData[year] = yearMonths;
      }
    } catch (e) {
      debugPrint("Firestore multi-year error: $e");
    }
    if (mounted) setState(() => _isMultiYearLoading = false);
  }
        ...
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                String key =
                    "${tempMonth.toString().padLeft(2, '0')}-$tempYear";
                if (!_selectedMultiMonths.contains(key)) {
                  setState(() => _selectedMultiMonths.add(key));
                  _fetchMultiMonthData();
                }
  ...
  Widget _buildMultiDateView() {
    ...
    _multiDateData.forEach((dateStr, rows) {
      ...
      for (var r in rows) {
        ...
        spots.add(FlSpot(hour, _extractParameterValue(r, _selectedParameter)));
      }
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            alignment: WrapAlignment.center,
            children:
                _multiDateData.keys.toList().asMap().entries.map((entry) {
              return _buildLegendItem(
                  entry.value, _chartColors[entry.key % _chartColors.length]);
            }).toList(),
          ),
        ],
      ),
                ...
                child: DropdownButtonFormField<int>(
                  ...
                  items: List.generate(
                      ...
                      (index) => DropdownMenuItem(
                          value: index + 1, child: Text(monthNames[index]))),
                  onChanged: (val) {
                    if (val != null) {
                      setState(() => _selectedMonthNumber = val);
                      _fetchParticularMonthData();
      ...
      child: Column(
        ...
        children: [
                ...
                child: SingleChildScrollView(
                  ...
                  child: Row(
                    children: _selectedMultiMonths.map((mKey) {
                      List<String> parts = mKey.split('-');
                      String label =
                          "${monthNames[int.parse(parts[0]) - 1]} ${parts[1]}";
                      return Padding(
                        padding: const EdgeInsets.only(right: 8.0),
                        child: Chip(
                          label: Text(label),
          ...
          Wrap(
            ...
            alignment: WrapAlignment.center,
            children:
                _multiMonthData.keys.toList().asMap().entries.map((entry) {
              return _buildLegendItem(
                  entry.value, _chartColors[entry.key % _chartColors.length]);
            }).toList(),
          ),
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            children: _multiYearData.keys.toList().asMap().entries.map((entry) {
              return _buildLegendItem(
                  entry.value.toString(), _chartColors[entry.key % _chartColors.length]);
            }).toList(),
          ),
        ],
      ),
        ...
        Expanded(
          child: Container(
            ...
            child: DropdownButton<String>(
              ...
              isExpanded: true,
              underline: const SizedBox(),
              items: _availableParameters.entries
